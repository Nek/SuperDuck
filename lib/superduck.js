...
var match = function(path, check) {
    return compose(check, accessor(path));
};
...
var makeDestruct = function(accs) {
    return function(o, f) {
        f.apply(o, compose(partial(map, function(acc){return acc(o);}), partial(map, accessor))(accs));
    };
};
...
var fcond = function(c,t,f) {
    return function(v) {
        if (c(v)) return t(v); else return f(v);
    };
};
...
var item = function(a, i) {
    return a[i];
};

var sizeEqZero = compose(
    partial(eq, 0), 
    _.size
);
...
var parsePath = function(p) {
    var recParsePath = function(p) {
        var nestedParse = function(p,e) {
            var result;
            return fcond(partial(eq, $), 
                         function() {return [e];},
                         compose(
                             fcond( 
                                 sizeEqZero,
        ...
        var result;
        if (is.Array(p)) result = [];
        else {
            result = 
                compose(
                    fcond(
                        function(ks) {return ks.length === 0;}, 
                        partial(identity, []), 
                        compose( 
                            partial(filter,function(e) {return e.length > 0;}), 
                            partial(map, partial(nestedParse, p)))
                    ), 
                    keys
                )(p);
        }
    ...
    return compose(partial(map, function(e) {return is.Array(e[0]) ? e[0] : e;}), recParsePath)(p);
};

var destructor = compose(makeDestruct, parsePath);
